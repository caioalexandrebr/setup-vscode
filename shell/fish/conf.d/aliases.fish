...
#
# `abbr` instead of `alias` where it makes sense: fish expands the abbreviation
# in place as you type, so you see the real command before running it and shell
# history stays readable. `alias` is used only where the expansion would be
# confusing or where a real function is needed.
